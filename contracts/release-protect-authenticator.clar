...
;; A decentralized access management and release protection contract
;; enabling secure, controlled access and verification for digital releases and sensitive resources.
...
(define-constant err-no-such-release (err u101))
(define-constant err-no-such-access-token (err u102))
(define-constant err-release-already-exists (err u103))
(define-constant err-token-already-exists (err u104))
(define-constant err-release-expired (err u105))
(define-constant err-release-completed (err u106))
(define-constant err-insufficient-permissions (err u107))
(define-constant err-not-verifier (err u108))
(define-constant err-invalid-access-level (err u109))
(define-constant err-invalid-expiration (err u110))
(define-constant err-token-already-revoked (err u111))
(define-constant err-verification-required (err u112))

;; Access Levels
(define-constant access-level-open u1)
(define-constant access-level-restricted u2)
